...
def motion(state, goal, params):
    ...
    K_z = 0.1
    state[2] += K_z * dist_to_goalZ * np.sign(goal[2] - state[2])
...
def flight_mission(drone, goal_x, goal_y, goal_z, params, collision_avoidance=True):
    if params.toFly: takeoff(drone, params.max_height)
...
def exploration_conveyer(drone, goal_x, goal_y, goal_z, params):
    ...
    for _ in range(params.num_missions):
        ...
        flight_mission(drone, goal_x, goal_y, goal_z, params, collision_avoidance=True)
        time.sleep(4)
